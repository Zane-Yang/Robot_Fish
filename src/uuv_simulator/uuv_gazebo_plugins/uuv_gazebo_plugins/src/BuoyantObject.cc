...
BuoyantObject::BuoyantObject(physics::LinkPtr _link)
{
  ...
  this->boundingBox = ignition::math::Box<double>(link->BoundingBox().Size());
#else
...
{
  double height = this->boundingBox.Size().Z();
  ...
  else
  {
    ...
    {
      this->waterLevelPlaneArea = this->boundingBox.Size().X() *
    ...
    if (z > height / 2.0) {
      ...
      curSubmergedHeight = this->boundingBox.Size().Z();
    } else {
...
/////////////////////////////////////////////////
void BuoyantObject::SetBoundingBox(const ignition::math::Box<double> &_bBox)
{
  this->boundingBox = _bBox;

  gzmsg << "Bounding box set to (X: " << this->boundingBox.Size().X()
      << ", Y: " << this->boundingBox.Size().Y()
      << ", Z: " << this->boundingBox.Size().Z() << ")" << std::endl;
}
