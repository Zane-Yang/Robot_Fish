    ...
    physics::LinkPtr _link) : BuoyantObject(_link)
{
  ...
  if (_sdf->HasElement("box"))
  {
        ...
        sdfModel->HasElement("height"))
    {
      ...
      ignition::math::Box boundingBox = ignition::math::Box<double>(ignition::math::Vector3d(width, length, height));
...
bool HMFossen::GetParam(std::string _tag, double& _output)
{
  ...
  if (!_tag.compare("volume"))
    ...
    _output = this->boundingBox.Size().Z();
  else if (!_tag.compare("bbox_width"))
    _output = this->boundingBox.Size().Y();
  else if (!_tag.compare("bbox_length"))
    _output = this->boundingBox.Size().X();
                   ...
                   : HMFossen(_sdf, _link)
{
  ...
  else
  {
    gzmsg << "HMSphere: Using the smallest length of bounding box as radius"
          << std::endl;
    this->radius = std::min(this->boundingBox.Size().X(),
                            std::min(this->boundingBox.Size().Y(),
                                     this->boundingBox.Size().Z()));
  }
                       ...
                       : HMFossen(_sdf, _link)
{
  ...
  else
  {
      gzmsg << "HMCylinder: Using the biggest length of bounding box as length"
            << std::endl;
      this->length = std::max(this->boundingBox.Size().X(),
                              std::max(this->boundingBox.Size().Y(),
                                       this->boundingBox.Size().Z()));
  }
  ...
  else
  {
    gzmsg << "HMCylinder: Using the direction of biggest length as axis"
          << std::endl;
    double maxLength = std::max(this->boundingBox.Size().X(),
                                std::max(this->boundingBox.Size().Y(),
                                         this->boundingBox.Size().Z()));
    if (maxLength == this->boundingBox.Size().X())
      this->axis = "i";
    else if (maxLength == this->boundingBox.Size().Y())
      this->axis = "j";
    else
      this->axis = "k";
  }
